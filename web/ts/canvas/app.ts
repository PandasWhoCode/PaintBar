...
export class PaintBar {
    ...
    private selectedArea: { x: number; y: number; width: number; height: number; backgroundState?: ImageData } | null;
    ...
    constructor(options: Partial<CanvasOptions> = {}) {
        ...
        this.transparentBgCanvas = document.getElementById('transparentBackgroundCanvas') as HTMLCanvasElement;
        this.transparentBgCtx = this.transparentBgCanvas.getContext('2d')!;

        this.opaqueBgCanvas = document.getElementById('opaqueBackgroundCanvas') as HTMLCanvasElement;
        ...
        this.overlayCanvas = document.getElementById('selectionOverlay') as HTMLCanvasElement;
        ...
        // Throttle utility
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        this.throttle = <T extends (...args: any[]) => void>(func: T, limit: number): T => {
        ...
        this.throttledMouseMove = this.throttle((e: MouseEvent) => this.handleMouseMove(e), 16);
    ...
    private initializeElements(): void {
        ...
        // Brush size
        this.brushSize = document.getElementById('brushSize') as HTMLInputElement | null;
    ...
    private initializeUI(): void {
        ...
        if (document.getElementById('textBtn')) {
            document.getElementById('textBtn')!.addEventListener('click', () => {
                if (this.toolManager.activeTool === (this.toolManager as unknown as { tools: Record<string, unknown> }).tools?.text) {
    ...
    private setupEventListeners(): void {
        ...
        if (brushSize) {
            brushSize.addEventListener('input', (e) => {
                this.lineWidth = this.calculateLineWidth(Number((e.target as HTMLInputElement).value));
                this.ctx.lineWidth = this.lineWidth;
                if (brushSizeLabel) {
        ...
        const triangleTypeSelect = document.getElementById('triangle-type') as HTMLSelectElement | null;
        ...
        if (triangleBtn && triangleTypeSelect) {
            ...
            triangleTypeSelect.addEventListener('change', (e) => {
                this.triangleType = (e.target as HTMLSelectElement).value as TriangleType;
            });
        ...
        const transparentCanvas = document.getElementById('transparentCanvas') as HTMLInputElement | null;
     ...
     */
    /* @internal — used by legacy shape drawing path */
    private drawShape(ctx: CanvasRenderingContext2D, points: Point[], shouldFill = false): void {
     ...
     */
    /* @internal — used by legacy shape drawing path */
    private drawCircleShape(ctx: CanvasRenderingContext2D, center: Point, radius: number, shouldFill = false): void {
        ctx.beginPath();
     ...
     */
    /* @internal — used by legacy shape drawing path */
    private calculateDistance(point1: Point, point2: Point): { dx: number; dy: number; distance: number; angle: number } {
     ...
     */
    private colorsMatch(color1: number[], color2: number[], tolerance = 1): boolean {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const responsiveCanvas = document.getElementById('responsiveCanvas') as HTMLInputElement;
    ...
    const handleDimensionChange = (changedInput: HTMLInputElement, otherInput: HTMLInputElement): void => {
        if (isSquareLocked) {
    ...
    startPaintBarBtn.addEventListener('click', () => {
        const options: Partial<CanvasOptions> = {
            ...
            minWidth: parseInt((document.getElementById('minWidth') as HTMLInputElement).value, 10),
            minHeight: parseInt((document.getElementById('minHeight') as HTMLInputElement).value, 10),
            maxWidth: parseInt((document.getElementById('maxWidth') as HTMLInputElement).value, 10),
            maxHeight: parseInt((document.getElementById('maxHeight') as HTMLInputElement).value, 10),
