...
export class FillTool extends GenericTool {
  ...
  private fill(point: Point): void {
    ...
    const imageData = ctx.getImageData(
      0,
      0,
      this.paintBar.canvas.width,
      this.paintBar.canvas.height,
    );
...
export class TextTool extends GenericTool {
  ...
  private setupEventListeners(): void {
    ...
    if (modalHeader) {
      ...
      modalHeader.addEventListener(
        "touchstart",
        (e) => this.startDragging(e as TouchEvent),
        { passive: false },
      );
    ...
    const textRotation = document.getElementById(
      "textRotation",
    ) as HTMLInputElement | null;
    ...
    const toggleStyleButton = (
      ...
      button.addEventListener("click", () => {
        (this.textState[property] as boolean) = !(this.textState[
          property
        ] as boolean);
  ...
  private initializeTextTool(x: number, y: number): void {
    ...
    if (
      ...
      if (italicBtn)
        italicBtn.classList.toggle("active", this.textState.italic);
      if (underlineBtn)
        underlineBtn.classList.toggle("active", this.textState.underline);
      if (strikeBtn)
        strikeBtn.classList.toggle("active", this.textState.strikethrough);
